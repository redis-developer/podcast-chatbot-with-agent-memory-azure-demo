...
export async function requestAndResponse(request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
  try {
    ...
    if (!message) return responses.badRequest('Message is required')
